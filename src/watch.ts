...
export type WacthCallback<T = any> = (
  value: T,
  prevValue: T,
  onCleanup: (cb: () => void) => void
) => any;

export type Watchable<T = any> = ValueWrapper<T> | (() => T);
...
export function watch<T extends readonly Watchable[]>(
  target: T,
  cb: WacthCallback<UnwrapWatchableList<Writable<T>>>,
  opts?: WatchOptions
): () => void;

export function watch<T extends Watchable[]>(
  target: T,
  cb: WacthCallback<UnwrapWatchableList<T>>,
  opts?: WatchOptions
): () => void;

export function watch(
  target: any,
  cb: WacthCallback<any>,
  opts: WatchOptions = {}
) {
  let getter: () => any;
  if (isArray(target)) {
...
function createWatchUpdaterAndCleaner(update: WacthCallback<any>) {
  let clean = noop;
