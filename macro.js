...
function transform({
  ...
  Object.keys(references).forEach(name => {
    if (references[name].length && name[0].toLowerCase() === name[0]) {
      importSpecifiers.push(t.importSpecifier(references[name][0].node, t.identifier(name)));
    }
  });

  if (importSpecifiers.length) {
    const importNode = t.importDeclaration(importSpecifiers, t.stringLiteral(path.resolve(__dirname, "runtime.js")));
  ...
  function transformModel(paths) {
    ...
    paths.forEach(modelPath => {
      ...
      if (modelPath.parentPath.isCallExpression()) {
        const configPath = modelPath.parentPath.get("arguments")[0]; // check config is { ... }
        ...
        if (configPath && configPath.isObjectExpression()) {
          configPath.get("properties").forEach(propPath => {
            ...
            if (propPath.get("value").isObjectExpression()) {
              if (!propPath.node.computed && propPath.node.key.name === "actions") {
          ...
          let ns = state.filename.replace(/^.*?models[/\\]/, "").replace(/(?:\.\.(?:\\+|\/)?)+/g, "").replace(/\\+/g, "/").replace(path.extname(state.filename), "");
          ...
          if (process.env.NODE_ENV !== "production") {
            configPath.node.properties.unshift(t.objectProperty(t.identifier("__prefix"), t.stringLiteral(ns)));
          }
        }
        ...
        function transformFunc(func) {
          ...
          func.traverse({
            ThisExpression(thisPath) {
              ...
              while (!thisScope.path.isFunction() || thisScope.path.isArrowFunctionExpression()) {
        ...
        function seperateObjectProperty(propPath) {
          const propValueIdent = modelPath.scope.generateUidIdentifier(propertyName(propPath.get("key")));
          modelPath.findParent(p => p.isStatement()).insertBefore(t.variableDeclaration("const", [t.variableDeclarator(propValueIdent, propPath.node.value)]));
        ...
        function transformObjectExpression(path, enableMangle) {
          ...
          path.get("value.properties").forEach(prop => {
            const localEnableMangle = enableMangle && !prop.node.computed && !path.get("key").isIdentifier({
              name: "state"
            });
            ...
            if (prop.get("value").isFunctionExpression()) {
              ...
              if (prop.node.kind === "method") {
                const propValueIdent = modelPath.scope.generateUidIdentifier(propertyName(prop.get("key")));
                const func = t.functionExpression(prop.get("key").isIdentifier() ? prop.node.key : null, prop.node.params, prop.node.body, prop.node.generator, prop.node.async);
                modelPath.findParent(p => p.isStatement()).insertBefore(t.variableDeclaration("const", [t.variableDeclarator(propValueIdent, func)]));
                prop.replaceWith(t.objectProperty(prop.node.key, propValueIdent, prop.node.computed, false, prop.node.decorators));
              }
